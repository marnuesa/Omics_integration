...
for (t in times) {
  for (s in stresses) {
    ...
    DMRsBinsCG = computeDMRs(metDataList[["NT"]],
                             ...
                             minCytosinesCount = 3,
                             minProportionDifference = 0.0001,
                             minGap = 300,
                             minSize = 50,
    ...
    DMRsCGList <- list("bins" = DMRsBinsCGMerged)
    # plot the local profile
    par(cex=0.9)
    par(mar=c(4, 4, 3, 1)+0.1)
    DMRplot = plotLocalMethylationProfile(methylationDataList[["NT"]],
                                methylationDataList[["stressed"]],
                                chr3Reg,
                                DMRsCGList,
                                conditionsNames = c("NT", "stressed"),
                                GEs,
                                windowSize = 300,
                                main="CG methylation")
    
    ## 4-WRITING REPORTS
    if (!dir.exists(paste0(WD, "/DMRs_all"))){
      dir.create(paste0(WD, "/DMRs_all")) }
    if (!dir.exists(paste0(WD, "/DMRs_all/plot"))){
      dir.create(paste0(WD, "/DMRs_all/plot"))
    }
    
  
    DMRsBinsCHGMerged = as.data.frame(DMRsBinsCHGMerged)
    write.table(DMRsBinsCHGMerged, paste0(WD, "/DMRs_all/", t, "-", s, "_DMRs_Bins_CHG.tsv"), sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
    DMRsBinsCHHMerged = as.data.frame(DMRsBinsCHHMerged)
    write.table(DMRsBinsCHHMerged, paste0(WD, "/DMRs_all/", t, "-", s, "_DMRs_Bins_CHH.tsv"), sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
    
    DMRsBinsCGMerged = DMRsBinsCGMerged[!duplicated(DMRsBinsCGMerged[,1:14]),]
    write.table(DMRsBinsCGMerged, paste0(WD, "/DMRs_all/", t, "-", s, "_DMRs_Bins_CG_duplicates_removed.tsv"), sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
    DMRsBinsCHGMerged = DMRsBinsCHGMerged[!duplicated(DMRsBinsCHGMerged[,1:14]),]
    write.table(DMRsBinsCHGMerged, paste0(WD, "/DMRs_all/", t, "-", s, "_DMRs_Bins_CHG_duplicates_removed.tsv"), sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
    DMRsBinsCHHMerged = DMRsBinsCHHMerged[!duplicated(DMRsBinsCHHMerged[,1:14]),]
    write.table(DMRsBinsCHHMerged, paste0(WD, "/DMRs_all/", t, "-", s, "_DMRs_Bins_CHH_duplicates_removed.tsv"), sep="\t", col.names=TRUE, row.names=FALSE, quote=FALSE)
    
    ggsave(paste0(WD, "/DMRs_all/plot/", t, "-", s,"methylation_profile.png") , plot = DMRplot, width = 8, height = 6, dpi = 300)
  }
}
